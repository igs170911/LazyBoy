...
export function App(){
  ...
  useEffect(()=>{const frame=desktopFrameRef.current;if(!frame?.contentWindow||!screenUrl)return;frame.contentWindow.postMessage({type:"lazyboy-view-only",viewOnly:viewOnlyFor(computer.controlHolder)},location.origin)},[computer.controlHolder,screenUrl,desktopReady]);
  useEffect(()=>{const listener=(event:MessageEvent)=>{if(event.origin!==location.origin||event.source!==desktopFrameRef.current?.contentWindow||event.data?.type!=="lazyboy-request-control"||!paneBotId)return;void setControl("user")};window.addEventListener("message",listener);return()=>window.removeEventListener("message",listener)},[paneBotId]);
  ...
  async function openLoginScreen(){
    ...
    // The mouse is the point of the login screen, so it comes over on the
    // optimistic handoff rather than a second round trip behind a spinner.
    await setControl("user",id);
  }
  ...
  async function setControl(holder:ComputerStatus["controlHolder"],botId:string|null=paneBotId){
    ...
    setHandingOff(true);
    pushViewOnly(viewOnlyFor(holder));
