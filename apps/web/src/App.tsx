...
// Refresh policy: the session's event stream decides when the transcript is
// stale, so the timer is only a backstop - it runs slowly while the stream is
// up and speeds back up if the stream drops. One turn can move several events,
// which is why they settle into a single refresh instead of one fetch each.
// The backstop is also the only freshness guarantee for the computer banner:
// computers have no events of their own, so this interval is how long a state
// change nobody sent a message for (idle park, a crash, another user taking
// control) can stay invisible. Four seconds is still lighter than the pre-push
// tick, which paid for the whole transcript plus a heartbeat every two.
const EVENT_SETTLE_MS=120;
const LIVE_POLL_MS=4000;
const FALLBACK_POLL_MS=2000;
// A heartbeat buys a 15 minute control lease and keeps the 10 minute idle
// cutoff from parking the computer, so a minute between beats is a wide margin.
...
function MessageTime({value}:{value:string}){
  ...
  return <time className="message-time" dateTime={date.toISOString()} title={date.toLocaleString(dateLocale())}>{clockTime(date,dateLocale())}</time>;
}
function DayDivider({value}:{value:string}){
  const date=new Date(value);
  if(!Number.isFinite(date.getTime()))return null;
  return <div className="day-divider" role="separator" aria-label={date.toLocaleDateString(dateLocale())}><time dateTime={date.toISOString()} title={date.toLocaleDateString(dateLocale(),{dateStyle:"full"})}>{dayLabel(date,new Date(),dateLocale(),{today:t("today"),yesterday:t("yesterday")})}</time></div>;
}
...
function localizeError(message:string){
  ...
  if(message==="AI 回應逾時（150 秒）")return t("aiTimeout");
  if(message==="run is not retryable")return t("errorRetryFailed");
...
type MessageChip={kind:string;site?:string;why?:string;name?:string;human?:string;cron?:string;reason?:string;turns?:number;limit?:number;code?:string;retryable?:boolean;runId?:string;turn?:number;step?:string|null};
function chipBlocks(blocks:unknown){if(!Array.isArray(blocks))return [] as MessageChip[];return blocks.flatMap(block=>{if(!block||typeof block!=="object")return [];const value=block as MessageChip;if(value.kind==="login"||value.kind==="schedule"||value.kind==="scheduleRun"||value.kind==="resume"||value.kind==="error")return [value];return []})}
function resumeTitle(reason?:string){return reason==="budget_exhausted"?t("resumeBudget"):reason==="loop_detected"?t("resumeLoop"):t("resumeMidTask")}
...
export function App(){
  ...
  const [sessionMenuOpen,setSessionMenuOpen]=useState(false); const [clearOpen,setClearOpen]=useState(false); const [sessionToDelete,setSessionToDelete]=useState<string|null>(null); const [remembered,setRemembered]=useState<Record<string,boolean>>({}); const [resumedChips,setResumedChips]=useState<Record<string,boolean>>({}); const [retriedRuns,setRetriedRuns]=useState<Record<string,boolean>>({});
  ...
  const pendingSendNonces=useRef(new Map<string,string>());
  const currentSessionRef=useRef(activeSessionId);currentSessionRef.current=activeSessionId;
  ...
  // The holder the server has not agreed to yet, when the current handoff
  // started, and a guard so a fast double click cannot fight itself over the
  // mouse. All three exist so the click, not the round trip, owns the answer.
  const expectedHolderRef=useRef<ComputerStatus["controlHolder"]|null>(null); const handoffAtRef=useRef(0); const controlBusyRef=useRef(false); const [controlBusy,setControlBusy]=useState(false);
  const [veil,setVeil]=useState<Veil>({label:null,leaving:false});
  ...
  const[memoryTargets,setMemoryTargets]=useState<Record<string,string>>({});
  const[pendingMemory,setPendingMemory]=useState<{message:Message;sessionId:string|null;members:RoomMember[]}|null>(null);
  const[memorySaveBusy,setMemorySaveBusy]=useState(false);
  const savingMemoryRef=useRef(false);
  useEffect(()=>setPendingMemory(null),[activeSessionId]);
  const memoryTargetId=activeRoom?(activeRoom.members.find(member=>member.id===memoryTargets[activeRoom.id])?.id||activeRoom.members[0]?.id):active?.id;
  const memoryBot=bots.find(bot=>bot.id===memoryTargetId);
  ...
  const desktopInteractive=computer.screenAvailable&&!viewOnlyFor(computer.controlHolder,computer.sharedInput);
  ...
  // Only the newest assistant reply can still be "continued": an older pause
  // belongs to a run that has already moved on.
  const lastAssistantSeq=messages.reduce((max,message)=>message.role==="assistant"?Math.max(max,message.seq??0):max,0);
  ...
  const refresh=useCallback(async()=>{if(!activeSessionId)return;const refreshSeq=++refreshSeqRef.current;let nextBusy:RoomMember[]=[];
    // Transcript delivery is independent of desktop health and screen discovery.
    // Resolve errors as values until joined below, so an early rejection cannot
    // become unhandled while the computer request is still in flight.
    const messagesJob=api<Message[]>(`/api/sessions/${activeSessionId}/messages`).then(next=>{if(refreshSeq===refreshSeqRef.current)setMessages(next);return null},error=>error);
    ...
    if(!computerBot){const messageError=await messagesJob;if(messageError)throw messageError;if(refreshSeq===refreshSeqRef.current){setBusyMembers(nextBusy);setComputer(blankComputer);setSkills([]);setScreenUrl(null)}return}
    ...
    const [messageError,screen,nextSkills]=await Promise.all([messagesJob,status.state==="running"?api<{url:string|null}>(`/api/computer/${computerBot}/screen`).catch(()=>({url:null})):Promise.resolve({url:null}),skillsJob]);
    if(refreshSeq!==refreshSeqRef.current)return;
    if(messageError)throw messageError;
    setBusyMembers(nextBusy);setComputer(status);setSkills(nextSkills);
    // A missing url is not a dead desktop. While it boots or wakes the mounted
    // frame keeps its VNC session instead of blacking out and reconnecting.
    setScreenUrl(current=>status.botId===computerBot?keepScreenUrl(current,screen.url,status.state):null);
    // Agreement with the server is what ends a handoff. The beat has a floor so
    // the swap is still visible on a LAN, and the ceiling timer still covers a
    // reply that never lands.
    if(expectedHolderRef.current&&status.controlHolder===expectedHolderRef.current){
      expectedHolderRef.current=null;
      const wait=handoffRemaining(handoffAtRef.current,Date.now());
      if(wait>0)window.setTimeout(()=>setHandingOff(false),wait);else setHandingOff(false);
    }
  ...
  useEffect(()=>{const panel=messageEndRef.current?.parentElement;if(panel&&panel.scrollHeight-panel.scrollTop-panel.clientHeight<180)panel.scrollTop=panel.scrollHeight},[replyDrafts]);
  ...
  // Live transcript: the event stream says when to look, so a reply appears as
  // soon as it is written. Polling stays as a backstop that cannot starve the
  // database, and a hidden tab stops re-reading until it comes back to the
  // foreground - which is itself a "what happened" moment worth refreshing on.
  useEffect(()=>{
    if(!activeSessionId||(!activeId&&!activeRoomId)){refreshSeqRef.current+=1;setMessages([]);setScreenUrl(null);if(!activeId&&!activeRoomId)setComputer(blankComputer);return}
    setReplyDrafts({});
    setScreenUrl(null);
    refresh().catch(e=>setError(localizeError(e.message)));
    const settle=createCoalescer(()=>{if(!document.hidden)refresh().catch(()=>{})},EVENT_SETTLE_MS);
    let live=false,opened=false,timer=0;
    const tick=()=>{if(!document.hidden)refresh().catch(()=>{});timer=window.setTimeout(tick,live?LIVE_POLL_MS:FALLBACK_POLL_MS)};
    timer=window.setTimeout(tick,FALLBACK_POLL_MS);
    const heartbeat=window.setInterval(()=>{const beat=roomsRef.current.find(room=>room.id===activeRoomId)?.members[0]?.id||activeId;if(beat)api(`/api/computer/${beat}/heartbeat`,{method:"POST",body:"{}"}).catch(()=>{})},HEARTBEAT_MS);
    const feed=subscribeToSession(activeSessionId,event=>{setReplyDrafts(current=>applyReplyEvent(current,event));if(!event.kind.startsWith("reply."))settle.kick()},{onStatus:connected=>{live=connected;if(connected&&opened)refresh().catch(()=>{});opened=true}});
    const resume=()=>{if(!document.hidden)refresh().catch(()=>{})};
    document.addEventListener("visibilitychange",resume);
    return()=>{window.clearTimeout(timer);window.clearInterval(heartbeat);document.removeEventListener("visibilitychange",resume);feed.close();settle.cancel();refreshSeqRef.current+=1};
  },[activeId,activeRoomId,activeSessionId,refresh]);
  useEffect(()=>{const listener=(event:MessageEvent)=>{if(event.origin!==location.origin||event.source!==desktopFrameRef.current?.contentWindow||!event.data)return;if(event.data.type==="lazyboy-desktop-clipboard"){const text=String(event.data.text||"");setDesktopClipboard(text);navigator.clipboard?.writeText(text).then(()=>setClipboardStatus(t("clipboardSynced"))).catch(()=>setClipboardStatus(t("clipboardSyncBlocked")))}if(event.data.type==="lazyboy-copy-request"&&desktopInteractive)void copySelection();if(event.data.type==="lazyboy-paste-text"&&typeof event.data.text==="string")pasteText(event.data.text);if(event.data.type==="lazyboy-mobile-key"&&typeof event.data.key==="string"&&/^(?:(?:ctrl|alt)\+)?(?:Return|BackSpace|Tab|Escape|Left|Right|Up|Down|[acvz])$/.test(event.data.key))queueDesktopInput({kind:"key",key:event.data.key});if(event.data.type==="lazyboy-paste-request"&&desktopInteractive)setClipboardOpen(true);if(event.data.type==="lazyboy-desktop-ready")setDesktopReady(true);if(event.data.type==="lazyboy-desktop-lost")setDesktopReady(false)};window.addEventListener("message",listener);return()=>window.removeEventListener("message",listener)});
  ...
  async function setRoomHost(botId:string){
    ...
    try{await api<Room>(`/api/rooms/${activeRoom.id}`,{method:"PATCH",body:JSON.stringify({hostBotId:botId})});await loadBots()}catch(error){setError(localizeError(error instanceof Error?error.message:t("operationFailed")))}
  }
  ...
  async function openLoginScreen(){
    ...
    await action(async()=>{try{await api(`/api/computer/${id}/boot`,{method:"POST",body:"{}"})}catch{/* already up */}});
    // Opening the shared screen never pauses a running task.
    if(!computer.sharedInput)await setControl("user",id);
  }
  ...
  async function send(event:FormEvent){
    ...
    const submittedDraft=draft,text=submittedDraft.trim(),files=pendingFiles,sessionId=activeSessionId;
    if((!active&&!activeRoom)||!sessionId||(!text&&files.length===0))return;
    // An uncertain HTTP result must reuse the original nonce on retry. Keep
    // the user's input until acknowledgement; a failed refresh is not a failed send.
    const key=JSON.stringify([sessionId,text,files.map(file=>file.id)]);
    const nonce=pendingSendNonces.current.get(key)||clientNonce();
    pendingSendNonces.current.set(key,nonce);
    sendingRef.current=true;setSendingMessage(true);
    try{await action(async()=>{
      const attachments=await Promise.all(files.map(async item=>({name:item.file.name,mimeType:item.file.type,content:await readAsBase64(item.file)})));
      await api(`/api/sessions/${sessionId}/messages`,{method:"POST",body:JSON.stringify({text,clientNonce:nonce,attachments})});
      pendingSendNonces.current.delete(key);
      if(currentSessionRef.current===sessionId){
        setDraft(current=>current===submittedDraft?"":current);
        setPendingFiles(current=>current.filter(item=>!files.some(sent=>sent.id===item.id)));
        historyIndexRef.current=null;historyDraftRef.current="";
      }
      sentHistoryRef.current.push(text||files[0]?.file.name||"");
  ...
  function pickMention(choice:{name:string}){const next=acceptMention(draft,caretAt,choice.name);setDraft(next.text);setCaretAt(next.caret);requestAnimationFrame(()=>composerRef.current?.focus())}
  ...
  // normal message: the backend folds it into the paused run instead of
  // starting a new task.
  async function continueRun(messageId:string){if(!activeSessionId||sendingRef.current||busy)return;sendingRef.current=true;const text=t("resumeSent");try{await action(async()=>{await api(`/api/sessions/${activeSessionId}/messages`,{method:"POST",body:JSON.stringify({text,clientNonce:clientNonce()})});setResumedChips(current=>({...current,[messageId]:true}));await loadSessions()})}finally{sendingRef.current=false}}
  async function retryRun(runId?:string){if(!runId||busy)return;await action(()=>api(`/api/runs/${runId}/retry`,{method:"POST",body:"{}"}));setRetriedRuns(current=>({...current,[runId]:true}))}
  ...
  async function saveMessageMemory(message:Message,botId:string,sessionId:string|null){
    if(savingMemoryRef.current||!message.body.trim())return;
    savingMemoryRef.current=true;setMemorySaveBusy(true);
    try{await api(`/api/bots/${botId}/memories`,{method:"POST",body:JSON.stringify({content:message.body,sessionId,sourceMessageId:message.id})});setRemembered(current=>({...current,[message.id]:true}));setPendingMemory(null)}catch(e){setError(e instanceof Error?localizeError(e.message):t("rememberFailed"))}finally{savingMemoryRef.current=false;setMemorySaveBusy(false)}
  }
  async function rememberMessage(message:Message){
    if(activeRoom&&!message.speakerBotId){setPendingMemory({message,sessionId:activeSessionId,members:activeRoom.members});return}
    const botId=message.speakerBotId||active?.id;
    if(botId)await saveMessageMemory(message,botId,activeSessionId);
  }
  ...
  function queueDesktopInput(input:{kind:"clipboard";text:string}|{kind:"key";key:string}){
    ...
    if(!botId||!desktopInteractive)return;
  ...
  async function changeComputer(operation:"boot"|"restart"|"stop"){
    ...
    // Stopping drops the pixels; starting keeps them. The viewer url never
    // changes, so the frame mounts straight away and noVNC dials in the moment
    // X answers, instead of booting only after the reply has already arrived.
    if(operation==="stop")setScreenUrl(null);else setScreenUrl(current=>current||viewerPath(botId));
    setComputer(current=>({...current,state:operation==="stop"?current.state:operation==="boot"&&current.state==="suspended"?"suspended":"booting"}));
    try{
      const status=await api<ComputerStatus>(`/api/computer/${botId}/${operation}`,{method:"POST",body:"{}",signal:AbortSignal.timeout(120_000)});
      if(currentPaneRef.current!==botId)return;
      setComputer(status);
      // Anything but running and the warm frame is a lie: drop it so the panel
      // shows what is really there instead of a desktop that will never return.
      if(status.state!=="running")setScreenUrl(null);
    }
    catch(error){
      const status=await api<ComputerStatus>(`/api/computer/${botId}/status`,{signal:AbortSignal.timeout(5_000)}).catch(()=>previous);
      if(currentPaneRef.current===botId){setComputer(status);if(status.state!=="running")setScreenUrl(null)}
      throw error;
    }
  }
  ...
  /** Tell the viewer, this instant, whether human input counts. The lease is
   * advisory inside the container, so this is the gate that actually moves the
   * mouse, and nobody should wait on a round trip to be able to click. */
  function pushViewOnly(viewOnly:boolean){desktopFrameRef.current?.contentWindow?.postMessage({type:"lazyboy-view-only",viewOnly},location.origin)}
  /** Take or release the screen. The button, the badge, the veil and the mouse
   * all move on the local copy; the server only has to agree with what is
   * already on screen, and if it refuses the read-back puts the mouse back. */
  async function setControl(holder:ComputerStatus["controlHolder"],botId:string|null=paneBotId){
    if(!botId||controlBusyRef.current)return;
    controlBusyRef.current=true;setControlBusy(true);
    expectedHolderRef.current=holder;
    setComputer(current=>({...current,controlHolder:holder,takeoverRequested:holder==="user"?false:current.takeoverRequested}));
    if(!computer.sharedInput)setHandingOff(true);
    pushViewOnly(viewOnlyFor(holder,computer.sharedInput));
    try{await api(`/api/computer/${botId}/${holder==="user"?"takeover":"release"}`,{method:"POST",body:"{}"})}
    catch(error){setError(localizeError(error instanceof Error?error.message:t("operationFailed")))}
    finally{
      // Read the truth back once: a refused takeover has to give the mouse up
      // again, and the status answer is who holds it now.
      await refresh().catch(()=>{});expectedHolderRef.current=null;controlBusyRef.current=false;setControlBusy(false);
    }
  }
  useEffect(() => {
  ...
  // The veil fades in with its label and fades back out through the same
  // mascot, so a fast handoff reads as a finished gesture rather than a frame
  // that was dropped. Status blips never get to strobe it.
  useEffect(()=>{setVeil(current=>nextVeil(overlayLabel,current))},[overlayLabel]);
  useEffect(()=>{if(!veil.leaving)return;const timer=setTimeout(()=>setVeil({label:null,leaving:false}),VEIL_FADE_MS);return()=>clearTimeout(timer)},[veil.leaving]);
  ...
  const hud=paneBot&&veil.label?<ComputerHud bot={paneBot} label={veil.label} leaving={veil.leaving}/>:null;
  ...
  return <AvatarLookProvider value={looks}><div className={`app-shell ${rightCollapsed?"right-collapsed":"right-open"}`}>
    ...
    <main className="chat-panel">
      <header className="topbar"><button type="button" className="icon-button mobile-menu" aria-label={t("chooseBot")} aria-expanded={mobileNav} aria-controls="conversation-sidebar" onClick={()=>setMobileNav(v=>!v)}><UseAnimations animation={menu} size={18} strokeColor="var(--ink)"/></button>{activeRoom?<><AvatarStack members={activeRoom.members} size={32} online thinkingIds={busyMembers.map(member=>member.id)}/><strong>{activeRoom.name}</strong><span className="host-chip-wrap" onClick={event=>event.stopPropagation()}><button type="button" className="host-chip" title={t("hostHint")} aria-haspopup="menu" aria-expanded={hostMenuOpen} onClick={()=>setHostMenuOpen(v=>!v)}><Megaphone size={13}/><span>{roomHost?roomHost.name:t("hostBadge")}</span></button>{hostMenuOpen&&<div className="host-menu" role="menu"><small className="host-menu-label">{t("hostMenuLabel")}</small>{activeRoom.members.map(member=><button type="button" role="menuitemradio" aria-checked={member.id===roomHostId} key={member.id} onClick={()=>void setRoomHost(member.id)}><Avatar lookId={member.id} name={member.name} color={member.avatarColor} shape={member.avatarShape} size={18}/><span>{member.name}</span>{member.id===roomHostId&&<b className="host-flag">{t("hostBadge")}</b>}</button>)}</div>}</span><SessionMenu sessions={sessions} activeSessionId={activeSessionId} open={sessionMenuOpen} setOpen={setSessionMenuOpen} busy={busy} onSelect={selectSession} onCreate={createSession} onDelete={id=>{setSessionMenuOpen(false);setSessionToDelete(id)}} onClear={()=>{setSessionMenuOpen(false);setClearOpen(true)}}/><span className="grow"/>{topTools}</>:active?<><Avatar lookId={active.id} name={active.name} color={active.avatarColor} shape={active.avatarShape} active online/><strong>{active.name}</strong><SessionMenu sessions={sessions} activeSessionId={activeSessionId} open={sessionMenuOpen} setOpen={setSessionMenuOpen} busy={busy} onSelect={selectSession} onCreate={createSession} onDelete={id=>{setSessionMenuOpen(false);setSessionToDelete(id)}} onClear={()=>{setSessionMenuOpen(false);setClearOpen(true)}}/><span className="grow"/>{topTools}</>:<><strong>{t("chooseBot")}</strong><span className="grow"/>{topTools}</>}</header>
      {callOpen&&voiceSettings?.enabled&&active&&activeSessionId&&!activeRoomId?<CallOverlay bot={active} sessionId={activeSessionId} takeover={computer.takeoverRequested||(!computer.sharedInput&&computer.controlHolder==="user")} onHangUp={()=>setCallOpen(false)} onTakeOver={()=>{setRightPart("computer");setRightCollapsed(false);setComputerOpen(true);if(active&&!computer.sharedInput)void setControl("user",active.id)}}/>:null}
      <div className="messages">{(activeRoom||active)&&messages.length===0?<div className="welcome">{activeRoom?<AvatarStack members={activeRoom.members} size={56} online/>:<Avatar lookId={active!.id} name={active!.name} color={active!.avatarColor} shape={active!.avatarShape} active online size={64}/>}<h1>{activeRoom?t("startRoomDiscussion",{name:activeRoom.name}):t("startBotWork",{name:active!.name})}</h1><p>{activeRoom?t("roomWillReply",{names:listJoin(activeRoom.members.map(member=>member.name))}):active!.description||t("botWelcome")}</p></div>:messages.map((message,index)=>{const spoken=message.role!=="user"&&Boolean(activeRoom);const speakerName=message.speakerName||(spoken?paneBot?.name:undefined);const speakerShape=(message.speakerShape||paneBot?.avatarShape||"blob") as AvatarShape;const files=messageFiles(message.blocks);const chips=chipBlocks(message.blocks);const hideBody=isAutoAttachCaption(message.body,files)||isAutoScheduleCaption(message.body,chips);const previous=index>0?messages[index-1]:null;const newDay=!previous||!sameDay(previous.createdAt,message.createdAt);return <Fragment key={message.id}>{newDay&&<DayDivider value={message.createdAt}/>}<div className={`message ${message.role} ${spoken?"spoken":""} ${files.length?"with-files":""}`}>{spoken&&<span className="msg-avatar"><Avatar lookId={message.speakerBotId||paneBot?.id||undefined} name={speakerName||"agent"} color={message.speakerColor||undefined} shape={speakerShape} size={22}/></span>}{spoken&&<b className="speaker" style={{color:message.speakerColor||undefined}}>{speakerName}</b>}{files.length>0&&<div className="msg-attachments">{files.map(file=><FileCard key={file.name} file={file}/>)}</div>}{chips.map((chip,index)=>chip.kind==="error"?<div className="login-chip error-chip" key={`${message.id}-error-${index}`}><div className="login-label">{errorTitle(chip.code)}</div><div className="error-actions">{errorActions(chip.code).map(next=>next==="retry"?<button type="button" className="primary" key="retry" title={t("errorRetryHint")} disabled={busy||!chip.runId||Boolean(retriedRuns[chip.runId])} onClick={()=>void retryRun(chip.runId)}>{retriedRuns[chip.runId||""]?t("errorRetried"):t("errorRetry")}</button>:next==="screen"?<button type="button" key="screen" onClick={()=>void openLoginScreen()}>{t("loginOpenScreen")}</button>:<button type="button" key="settings" onClick={()=>openAccount("model")}>{t("errorOpenSettings")}</button>)}<RunProbe runId={chip.runId||null} align="end" label={t("errorDetails")}><span className="error-probe" title={t("errorDetails")}><Info/></span></RunProbe></div></div>:chip.kind==="resume"?<div className="login-chip resume-chip" key={`${message.id}-resume-${index}`}><div className="login-label">{resumeTitle(chip.reason)}</div>{(chip.limit||0)>0?<div className="login-why">{t("resumeProgress",{turns:chip.turns||0,limit:chip.limit||0})}</div>:null}{resumedChips[message.id]||(message.seq??0)<lastAssistantSeq?null:<div className="resume-actions"><button type="button" className="primary" disabled={busy} onClick={()=>void continueRun(message.id)}>{t("resumeContinue")}</button><button type="button" className="outline" disabled={busy} onClick={()=>setResumedChips(current=>({...current,[message.id]:true}))}>{t("resumeStop")}</button></div>}</div>:chip.kind==="login"?<div className="login-chip" key={`${message.id}-login-${index}`}><div className="login-label">{t("loginNeedsYou")}</div><div className="login-site">{chip.site||message.body}</div>{chip.why?<div className="login-why">{t("loginWhy",{why:chip.why})}</div>:null}<button type="button" className="primary" onClick={()=>void openLoginScreen()}>{t("loginOpenScreen")}</button></div>:chip.kind==="schedule"?<div className="sched-chip" key={`${message.id}-sched-${index}`}><div className="sched-label">{t("scheduleChip")}</div><strong>{chip.name}</strong><small>{scheduleWhen(chip.cron,chip.human)}</small></div>:<div className="sched-chip" key={`${message.id}-run-${index}`}><div className="sched-label">{t("scheduleRunChip")}</div><strong>{chip.name}</strong><small>{scheduleWhen(chip.cron,chip.human)}</small></div>)}{!hideBody&&(message.role==="assistant"?<div className="message-stack"><div className="message-body md"><ChatMarkdown>{message.body}</ChatMarkdown></div>{message.body.trim()?<CopyMessageButton text={message.body}/>:null}{message.runId&&<RunProbe runId={message.runId} label={t("messageRunDetails")}><span className="message-run-details"><Info/>{t("messageRunDetails")}</span></RunProbe>}</div>:<span className="message-body">{message.body}</span>)}{!hideBody&&message.body.trim()&&<button type="button" className={`remember-msg ${remembered[message.id]?"saved":""}`} title={remembered[message.id]?t("remembered"):t("remember")} disabled={!!remembered[message.id]} onClick={()=>void rememberMessage(message)}><UseAnimations animation={bookmark} size={14} strokeColor="var(--muted)"/></button>}<MessageTime value={message.createdAt}/>{message.replyBots&&message.replyBots.length>0&&<div className="route-note">{message.role==="user"?t("routedTo",{names:listJoin(message.replyBots.map(reply=>reply.name))}):t("handedTo",{name:message.replyBots[0].name})}</div>}</div></Fragment>})}{pausedForUser&&paneBot&&<div className="pause-banner" role="status"><span>{computer.sharedInput?t("sharedNeedsUser",{name:paneBot.name}):computer.controlHolder==="user"?t("pausedUserControl",{name:paneBot.name}):t("pausedNeedsUser",{name:paneBot.name})}{(computer.queuedRuns||0)>0&&<> {t("queuedMessages",{count:computer.queuedRuns||0})}</>}</span>{computer.sharedInput||computer.controlHolder==="user"?<button type="button" className="primary" disabled={busy||controlBusy} onClick={()=>void setControl("none",paneBot.id)}>{computer.sharedInput?t("doneContinue"):t("releaseAndContinue")}</button>:<button type="button" className="primary" disabled={busy||controlBusy} onClick={()=>void setControl("user",paneBot.id)}>{t("takeOverNow")}</button>}</div>}{teaching&&active&&<div className="teach-banner recording" role="status"><span><i className="record-dot live"/>{t("teachingLive",{goal:teaching.goal})}<small>{t("teachingHint")}{teaching.eventCount>0&&<> · {t("teachingCaptured",{count:teaching.eventCount})}</>}</small></span><button type="button" className="primary" disabled={busy} onClick={()=>void stopTeaching()}>{t("finishDemo")}</button><button type="button" className="outline" disabled={busy} onClick={()=>void cancelTeaching()}>{t("cancel")}</button></div>}{drafting&&<div className="teach-banner" role="status"><UseAnimations animation={loading} size={18} wrapperStyle={{display:"inline-block",verticalAlign:"middle"}}/><span>{t("distilling",{goal:drafting.goal})}</span></div>}{skillDraft&&active&&<SkillDraftCard key={skillDraft.id} skill={skillDraft} busy={busy} onSave={(name,playbook)=>void saveSkill(skillDraft,name,playbook)} onTest={(name,playbook)=>void testSkill(skillDraft,name,playbook)} onDiscard={()=>void discardSkill(skillDraft)} onEdit={()=>setEditingSkillId(skillDraft.id)} onExport={(name,playbook)=>downloadSkill(name,skillDraft.goal,playbook)}/>}{Object.values(replyDrafts).filter(reply=>reply.text&&(!reply.messageId||!messages.some(message=>message.id===reply.messageId))).map(reply=>{const speaker=bots.find(bot=>bot.id===reply.botId);return <div className={`message assistant ${activeRoom?"spoken":""}`} key={reply.runId} aria-busy={!reply.messageId}>{activeRoom&&speaker&&<><span className="msg-avatar"><Avatar lookId={speaker.id} name={speaker.name} color={speaker.avatarColor} shape={speaker.avatarShape} online/></span><span className="speaker">{speaker.name}</span></>}<div className="message-stack"><div className="message-body md"><ChatMarkdown>{reply.text}</ChatMarkdown></div></div></div>})}<div ref={messageEndRef} aria-hidden="true"/></div>
    ...
    {computerOpen&&paneBot&&<div className="computer-overlay"><header><div><RunProbe runId={computer.busyRunId||computer.waitingRunId}><Avatar lookId={paneBot.id} name={paneBot.name} color={paneBot.avatarColor} shape={paneBot.avatarShape} active online/></RunProbe><strong>{modeLabel(computer.mode)}</strong><span className={`control-badge ${teaching?"teaching":""}`}>{teaching?t("teachingBadge"):computer.sharedInput?t("sharedDesktop"):computer.controlHolder==="user"?t("userControlling"):computer.usingComputer?t("aiReadOnly"):t("readOnly")}</span></div><div>{teaching?<div className="computer-actions"><button className="primary" disabled={busy} onClick={()=>void stopTeaching()}>{t("finishDemo")}</button><button className="outline" disabled={busy} onClick={()=>void cancelTeaching()}>{t("cancel")}</button></div>:<ControlButtons computer={computer} busy={busy} pending={controlBusy} action={action} active={paneBot} sessionId={activeSessionId} onBoot={startBoot} onRestart={restartComputer} onStop={stopComputer} onTakeOver={()=>void setControl("user")} onRelease={()=>void setControl("none")}/>}<button className="icon-button" onClick={pasteClipboard} disabled={!desktopInteractive} title={t("pasteClipboard")}><ClipboardPaste/></button><button className="icon-button" onClick={copyClipboard} disabled={!desktopInteractive||!desktopClipboard} title={t("copyDesktopClipboard")}><UseAnimations animation={copy} size={18} strokeColor="var(--ink)"/></button><button className="icon-button" onClick={()=>setComputerOpen(false)}><X/></button></div></header><div className="overlay-screen"><div className="overlay-desktop">{frame}{hud}</div></div>{error&&<div className="overlay-error">{error}</div>}</div>}
    ...
    {pendingMemory&&<div className="modal-backdrop" onClick={()=>!memorySaveBusy&&setPendingMemory(null)}><div className="dialog" role="dialog" aria-modal="true" aria-labelledby="memory-destination-title" onClick={e=>e.stopPropagation()}>
      <div className="dialog-title"><h2 id="memory-destination-title">{t("memoryChooseAgent")}</h2><button type="button" aria-label={t("cancel")} disabled={memorySaveBusy} onClick={()=>setPendingMemory(null)}><X/></button></div>
      <p className="dialog-lead">{t("memoryChooseAgentHelp")}</p>
      <div className="memory-destinations">{pendingMemory.members.map(member=><button type="button" className="outline" disabled={memorySaveBusy} key={member.id} onClick={()=>void saveMessageMemory(pendingMemory.message,member.id,pendingMemory.sessionId)}><Avatar lookId={member.id} name={member.name} color={member.avatarColor} shape={member.avatarShape} size={24}/>{member.name}</button>)}</div>
    </div></div>}
...
function MemoryPane({bot,changed}:{bot:Bot;changed:()=>Promise<void>}){
  ...
  const load=useCallback(async()=>{const version=++loadVersion.current;const[result,nextStatus]=await Promise.all([api<MemoryItem[]>(`/api/bots/${bot.id}/memories`),api<MemoryStatus>(`/api/bots/${bot.id}/memories/status`)]);if(version===loadVersion.current){setItems(result);setStatus(nextStatus)}},[bot.id]);
  useEffect(()=>{
    let mounted=true;
    const refresh=()=>{if(document.visibilityState!=="hidden")void load().catch(e=>{if(mounted)setError(e instanceof Error?localizeError(e.message):t("loadFailed"))})};
    refresh();const timer=window.setInterval(refresh,5000);
    window.addEventListener("focus",refresh);
    return()=>{mounted=false;++loadVersion.current;window.clearInterval(timer);window.removeEventListener("focus",refresh)};
  },[load]);
  ...
  return <div className="memory-pane">
    ...
    {status&&<p className="memory-help" role="status">{!status.globallyEnabled?t("monitorMemoryDisabled"):!enabled?t("memoryAgentDisabled"):t(status.embeddingStatus==="ready"?"memorySemanticReady":status.embeddingStatus==="unavailable"?"memorySemanticUnavailable":"memorySemanticBusy")}<br/>{t("memoryIndexedCount",{indexed:status.indexedCount,total:status.storedCount})}</p>}
    ...
    <div className="memory-list">{items.length===0?<p>{t("noMemory")}</p>:visible.length===0?<p>{t("noMatchingMemory")}</p>:visible.map(item=><div className="memory-row" key={item.id}>
      ...
      <small><time dateTime={item.updatedAt} title={new Date(item.updatedAt).toLocaleString()}>{inboxTime(item.updatedAt)}</time> · {t("memoryRevision",{revision:item.revision})} · {t(item.sourceMessageId?"memoryFromMessage":item.sourceRunId?"memoryFromAgent":"memoryFromManual")}</small>
      <details><summary>{t("memoryDetails")}</summary><dl>
        <dt>{t("memoryCreated")}</dt><dd>{new Date(item.createdAt).toLocaleString()}</dd>
        <dt>{t("memoryUpdated")}</dt><dd>{new Date(item.updatedAt).toLocaleString()}</dd>
        {item.sessionId&&<><dt>{t("memorySourceSession")}</dt><dd>{item.sessionId}</dd></>}
        {item.sourceMessageId&&<><dt>{t("memorySourceMessage")}</dt><dd>{item.sourceMessageId}</dd></>}
        {item.sourceRunId&&<><dt>{t("memorySourceRun")}</dt><dd>{item.sourceRunId}</dd></>}
      </dl></details>
...
function ComputerHud({bot,label,leaving}:{bot:{id:string;name:string;avatarColor?:string;avatarShape?:AvatarShape};label:string;leaving?:boolean}){
  const sep=getLocale()==="en"?": ":"：";
  return <div className={`computer-hud ${leaving?"is-leaving":""}`} role="status" aria-label={`${bot.name}${sep}${label}`}><span className="computer-signal" aria-hidden="true"><span className="computer-signal-face"><i/><i/></span></span><span className="computer-hud-label">{label}</span></div>
}
...
function ControlButtons({computer,busy,pending,action,active,sessionId,onBoot,onRestart,onStop,onTakeOver,onRelease}:{computer:ComputerStatus;busy:boolean;action:(w:()=>Promise<unknown>)=>Promise<void>;active:Bot;sessionId?:string|null;onBoot?:()=>Promise<void>;onRestart?:()=>Promise<void>;onStop?:()=>Promise<void>;pending?:boolean;onTakeOver:()=>void;onRelease:()=>void}){
  ...
  return <div className="computer-actions">
    {!running?<button className="primary" disabled={busy||booting} onClick={()=>run("boot",onBoot)}>{booting?t("bootingProgress"):t("openComputer")}</button>
      :computer.sharedInput?(computer.takeoverRequested||computer.controlHolder==="user"?<button className="primary" disabled={busy||pending} onClick={onRelease}>{t("doneContinue")}</button>:null)
      :computer.controlHolder==="user"?<button className="primary" disabled={busy||pending} onClick={onRelease}>{t("releaseControl")}</button>
      :<button className="primary" disabled={busy||pending} onClick={onTakeOver}>{working?t("takeOverNow"):t("takeControl")}</button>}
...
function ControlBar(props:{active:Bot;computer:ComputerStatus;busy:boolean;action:(w:()=>Promise<unknown>)=>Promise<void>;paste:()=>void;copy:()=>void;sessionId?:string|null;onBoot?:()=>Promise<void>;onRestart?:()=>Promise<void>;onStop?:()=>Promise<void>;pending?:boolean;onTakeOver:()=>void;onRelease:()=>void}){const interactive=props.computer.screenAvailable&&!viewOnlyFor(props.computer.controlHolder,props.computer.sharedInput);return <div className="control-bar"><ControlButtons {...props}/><button className="icon-button" disabled={!interactive} title={t("pasteClipboard")} onClick={props.paste}><ClipboardPaste/></button><button className="icon-button" disabled={!interactive} title={t("copyDesktopClipboard")} onClick={props.copy}><UseAnimations animation={copy} size={18} strokeColor="var(--ink)"/></button></div>}
