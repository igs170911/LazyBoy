...
export function App(){
  ...
  const refresh=useCallback(async()=>{if(!activeSessionId)return;const refreshSeq=++refreshSeqRef.current;let nextBusy:RoomMember[]=[];
    ...
    setBusyMembers(nextBusy);setComputer(status);setMessages(nextMessages);setSkills(nextSkills);
  ...
  useEffect(()=>{
    ...
    const feed=subscribeToSession(activeSessionId,()=>settle.kick(),{onStatus:connected=>{live=connected;if(connected&&opened)refresh().catch(()=>{});opened=true}});
